...
// We re-export the Express `app` created in src/server.ts.
...
app.use((req: Request, res: Response, next: NextFunction) => {
  ...
  res.on("finish", () => {
    if (res.statusCode >= 400)
      console.warn(`[${res.statusCode}] ${req.method} ${req.path} — ${Date.now() - start}ms`);
  });
  next();
});
...
app.get("/auth/callback", (req: Request, res: Response) => {
  res.send(`<!DOCTYPE html><html><head><title>NEXUS — Auth</title>
...
app.get("/api/profile", requireAuth, async (req: any, res: Response) => {
  const { data, error } = await supabaseAdmin.from("user_profiles").select("*").eq("id", req.user.id).single();
...
app.patch("/api/profile", requireAuth,
  ...
  async (req: any, res: Response) => {
    ...
    const { data, error } = await supabaseAdmin.from("user_profiles").update(updates).eq("id", req.user.id).select().single();
    if (error) return res.status(400).json({ error: error.message });
    res.json(data);
  }
);
...
app.get("/api/agent-logs",
  ...
  async (req: Request, res: Response) => {
    ...
    let q = supabaseAdmin.from("agent_logs").select("*").order("created_at", { ascending: false }).limit(limit);
...
app.get("/api/rwa-assets", async (_req: Request, res: Response) => {
  const { data, error } = await supabaseAdmin.from("rwa_assets").select("*").eq("status", "active").order("apy", { ascending: false });
  if (error) return res.status(400).json({ error: error.message });
  res.json(data);
});

app.get("/api/rwa-assets/:id", param("id").isUUID(), validate, async (req: Request, res: Response) => {
  const { data, error } = await supabaseAdmin.from("rwa_assets").select("*").eq("id", req.params.id).single();
...
app.get("/api/transactions", requireAuth,
  ...
  async (req: any, res: Response) => {
    ...
    const { data, error } = await supabaseAdmin.from("transactions").select("*").eq("user_id", req.user.id).order("created_at", { ascending: false }).limit(limit);
    if (error) return res.status(400).json({ error: error.message });
    res.json(data);
  }
);
...
app.get("/api/agent-policies", requireAuth, async (req: any, res: Response) => {
  const { data, error } = await supabaseAdmin.from("agent_policies").select("*").eq("user_id", req.user.id);
  if (error) return res.status(400).json({ error: error.message });
  res.json(data);
});

app.put("/api/agent-policies/:agent", requireAuth,
  ...
  async (req: any, res: Response) => {
    ...
    updates.updated_at = new Date().toISOString();
    const { data, error } = await supabaseAdmin.from("agent_policies").upsert(updates).select().single();
    if (error) return res.status(400).json({ error: error.message });
    res.json(data);
  }
);
...
app.get("/api/alerts", requireAuth, async (req: any, res: Response) => {
  const { data, error } = await supabaseAdmin.from("price_alerts").select("*").eq("user_id", req.user.id).order("created_at", { ascending: false });
  if (error) return res.status(400).json({ error: error.message });
  res.json(data);
});

app.post("/api/alerts", requireAuth,
...
app.delete("/api/alerts/:id", requireAuth, param("id").isUUID(), validate, async (req: any, res: Response) => {
  const { error } = await supabaseAdmin.from("price_alerts").delete().eq("id", req.params.id).eq("user_id", req.user.id);
  if (error) return res.status(400).json({ error: error.message });
